...
{
	/// <summary>
	/// Description of GKCPProtocol.
	/// </summary>
	[Protocol("Grow Kill Curve Plaiting","Ofer Fridman","Kill courve by plaiting.")]
	public class GKCPProtocol:Protocol
	{
		DateTime KillStartTime;
		LogInGoogleDocs myLogInGoogleDocs;
		
		public new GKCPProtocolParameters ProtocolParameters
		{
			get{return (GKCPProtocolParameters) base.ProtocolParameters;}
			set{base.ProtocolParameters = value;}
		}
		
		public GKCPProtocol(GKCPProtocolParameters ProtocolParameters):base((ProtocolParameters)ProtocolParameters)
		{
		...
		protected override void DoWork( )
		{
			...
			if (!ProtocolParameters.StartInKill)
			{
				...
				ChangeState(new GKCPGrow1State(ProtocolParameters.Grow1Time));
				...
				ChangeState(new GKCPDilut1State(ProtocolParameters.LicPlatePosition,ProtocolParameters.NumberOfSamples,ProtocolParameters.SharedResourcesFilePath));
			...
			ChangeState(new GKCPStartKillState(ProtocolParameters.LicPlatePosition,ProtocolParameters.NumberOfSamples,ProtocolParameters.AMPPosision));
			}
			...
			{				
				...
				if (TimeOfWait>0)
				{
					...
					ChangeState(new GKCPWaitState(TimeOfWait));
				}
				...
				ChangeState(new GKCPSampleState(ProtocolParameters.LicPlatePosition,ProtocolParameters.NumberOfSamples,firstSampleEppendorfInd));
		...
		private void Whait4OD(int Row)
		{
			...
			GKCPReadBackroundState ReadBackroundState = new GKCPReadBackroundState(ProtocolParameters.LicPlatePosition,Row,ProtocolParameters.NumberOfSamples);
			...
			double[] NetOD = new double[ProtocolParameters.NumberOfSamples];
			//Wait for the first exponential well to reach Dilution Net OD
			...
			ChangeState(new GKCPWait2ODReadState(ProtocolParameters.Time4TheFirstODTest));
			do
			{
				...
				GKCPReadODState ReadODState = new GKCPReadODState(ProtocolParameters.LicPlatePosition,Row,ProtocolParameters.NumberOfSamples);
				...
				if(NetOD.Max()<=ProtocolParameters.NetODtoDilute)
				{
					...
					ChangeState(new GKCPWait2ODReadState(TimeTillNextRead/60));
				}
			}
		...
		public static new List<Type> ProtocolStates()
		{
			return new List<Type>{
				typeof(GKCPGrow1State),
				typeof(GKCPDilut1State),
				typeof(GKCPStartKillState),
				typeof(GKCPSampleState),
				typeof(GKCPWaitState),
				typeof(GKCPReadBackroundState),
				typeof(GKCPWait2ODReadState),
				typeof(GKCPReadODState)
			};
		}
		#endregion
